...
public interface DemandeRepository extends JpaRepository<Demande, String> {
	...
	List<Demande> findByDemandeur_IdDemandeur(String idDemandeur);

	@Query("""
			    SELECT d
			    FROM Demande d
			    JOIN d.demandeur demandeur
			    JOIN demandeur.passports passport
			    WHERE passport.numero = :numeroPassport
			""")
	List<Demande> findByDemandeurPassportNumero(String numeroPassport);
